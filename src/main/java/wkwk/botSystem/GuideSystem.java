    ...
    public void run() {
        ...
        api.addMessageCreateListener(e -> {
            if (e.getMessageAuthor().asUser().isPresent() && !e.getMessageAuthor().asUser().get().isBot() && e.getServer().isPresent()) {
                ...
                if (e.getServer().get().getOwner().isPresent() && e.getMessageContent().startsWith(">") && e.getMessageAuthor().asUser().isPresent() && e.getServer().get().getPermissions(e.getMessageAuthor().asUser().get()).getAllowedPermission().contains(PermissionType.ADMINISTRATOR) || e.getServer().get().getOwner().get().getId() == e.getMessageAuthor().asUser().get().getId() && dao.getNoSlashCommandServer(serverId)) {
